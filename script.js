  ...
  addEventBtn = document.querySelector(".leggTil-event"),
  addEventWrapper = document.querySelector(".leggTil-event-wrapper "),
  addEventCloseBtn = document.querySelector(".close "),
  addEventTittel = document.querySelector(".event-navn "),
  addEventFra = document.querySelector(".event-tid-fra "),
  addEventTil = document.querySelector(".event-tid-til "),
  addEventSubmit = document.querySelector(".leggTil-event-btn ");
...
function initKalender() {
  ...
  const prevLastDag = new Date(year, month, 0);

  // Finner antall dager i forrige måned og antall dager i denne måneden
  const prevDager = prevLastDag.getDate();
  ...
  const nextDager = 7 - lastDag.getDay() - 1;
  ...
  for (let x = dag; x > 0; x--) {
    dager += `<div class="dag prev-dato">${prevDager - x + 1}</div>`;
  }
  ...
  for (let j = 1; j <= nextDager; j++) {
    dager +=
 `<div class="dag next-dato">${j}</div>`;
  }
...
function addListner() {
  ...
  dager.forEach((dag) => {
    dag.addEventListener("click", (e) => {
      ...
      if (e.target.classList.contains("prev-dato")) {
        prevMonth();
        ...
        setTimeout(() => {
          ...
          dager.forEach((dag) => {
            if (
              !dag.classList.contains("prev-dato") &&
      ...
      // Hvis den klikkede dagen tilhører neste måned, går vi til neste måned og markerer riktig dag
      else if (e.target.classList.contains("next-dato")) {
        nextMonth();
        ...
        setTimeout(() => {
          ...
          dager.forEach((dag) => {
            if (
              !dag.classList.contains("next-dato") &&
              dag.innerHTML === e.target.innerHTML
            ) {
              dag.classList.add("active");
            }
...
// Legger til en lytter på "input"-hendelsen på addEventTittel-elementet
  addEventTittel.addEventListener("input", (e) => {
    // Begrenser teksten som brukeren skriver inn til maksimalt 60 tegn
    addEventTittel.value = addEventTittel.value.slice(0, 60);
  });

//legger til en lytter på input-feltet med ID "addEventFra".
addEventListener("input", (e) => {
  // Fjerner alle tegn som ikke er tall eller kolon
  addEventFra.value = addEventFra.value.replace(/[^0-9:]/g, "");
  
  // Legger til kolon etter to tall hvis det ikke finnes allerede
  if (addEventFra.value.length === 2) {
    addEventFra.value += ":";
  }
  
  // Begrenser lengden på input-feltet til 5 tegn
  if (addEventFra.value.length > 5) {
    addEventFra.value = addEventFra.value.slice(0, 5);
  }
});

//funksjon som legger til hendelser til eventsArr
addEventSubmit.addEventListener("click", () => {
  const eventTittel = addEventTittel.value;
  const eventTidFra = addEventFra.value;
  const eventTidTil = addEventTil.value;
...
const tidFra = convertTid(eventTidFra);  
// Konverterer tiden fra tekststreng til et tidsobjekt
const tidTil = convertTid(eventTidTil);  
// Konverterer tiden til tekststreng til et tidsobjekt
...
//gjør om tiden til et 24-timers format
let tidArr = tid.split(":");
let tidHour = tidArr[0];
let tidMin = tidArr[1];
return tid;
} 
